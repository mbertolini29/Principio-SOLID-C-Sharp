using OpenClose;

//y aca en vez de Employee, crearias una interfaz... new List<IEmployee>()
ShowSalaryMonthly(new List<Employee>() {
    new EmployeeFullTime("Pepito Pérez", 160),
    new EmployeePartTime("Manuel Lopera", 180),
    new EmployeeContrator("Manuel Pérez", 200)
});

//esto seria un ejemplo de metodo generico, 
//porque sirve para cada clase de empleado, sea fulltime, parttime or contractor
//
void ShowSalaryMonthly(List<Employee> employees) 
{
    foreach (var employee in employees)
    {
        Console.WriteLine($"Empleado: {employee.Fullname}, Pago: {employee.CalculateSalaryMonthly():C1} ");        
    }
}